	...
	<head>
		...
		<script type="text/javascript">
			$(function(){
				$.ajax({
					type:"POST",
					url:"<%=basePath%>feedback/check?pageindex=1",
					dataType:"json",     
					contentType:"application/json",
					success:function(data){
						$("#datas").children().remove();
						var list= data.result.feedbacks;
						if(list.length!=0){
							for(var index=0;index<list.length;index++){
								var feedback = list[index];
								createTable(feedback.user.client_id,feedback.user.username,feedback.user.email,feedback.content,feedback.created_at);
							}
			...
			function searchflush(pageindex){
				...
				$.ajax({
					type:"POST",
					url:"<%=basePath%>feedback/checkFeedBack?pageindex="+parseInt(pageindex),
					dataType:"json",     
					contentType:"application/json",     
				...
				$.ajax({
					...
					success:function(data){
						...
						if(list.length!=0){
							...
							$("#datas").children().remove()
							$("#datas").append("<tr></tr>");
							$("#datas").children(":last").append("<td colspan=4 align='center'><strong class='red'>没有更多数据了！</strong></td>");
							$("#account").html(data.result.account)
							createPage(1,1);
						}
					}
			...
			*
			*/
			function createTable(client_id,username,email,content,created_at){
				$("#datas").append("<tr class='"+client_id+"'></tr>");
				$("#datas").children(":last").append("<td>"+username+"</td>");
				$("#datas").children(":last").append("<td>"+email+"</td>");
				$("#datas").children(":last").append("<td ><textarea class='form-control' readonly id='reason' col='8' rows='3'>"+content+"</textarea></td>");
				$("#datas").children(":last").append("<td>"+created_at+"</td>");
			}
                            ...
                            <div class="col-xs-12">
								...
								<div class="table-responsive">
									<table id="sample-table-2" class="table table-striped table-bordered table-hover">
										<thead>
											<tr>
												<th class="hidden-280">昵称</th>
												<th class="hidden-380">邮箱</th>
												<th class="hidden-580">反馈内容</th>
												<th>反馈时间</th>
											</tr>
										</thead>
										<tbody id="datas">													
