				...
				<div class="main-content">
					<div class="breadcrumbs" id="breadcrumbs">
						...
						<ul class="breadcrumb">
							...
							<li class="orange">留存分析</li>
						</ul><!-- .breadcrumb -->
					</div>
					<div class="page-content">
						<div class="page-header">
							<h1>
								<span class="orange2">留存分析</span>
							</h1>
						</div><!-- /.page-header -->
						<div class="row">
...
<![endif]-->
		...
		<script type="text/javascript">
			$(function(){
				...
				$.ajax({
					type:"POST",
					url:"<%=basePath%>statistic/remainUser",
					dataType:"json",     
					contentType:"application/json",
					data:JSON.stringify(jsonnull),
					success:function(data){
						var remainPercents = data.result.remainPercents;
						var numbers = data.result.numbers;
						var duration = buildTableFront(new Date(showdate(-7)),new Date(showdate(0)));
						for(var index=0;index<remainPercents.length;index++){
							$("#datas").append("<tr></tr>");
							$("#datas").children(":last").append("<td>"+duration[index]+"</td>")
							$("#datas").children(":last").append("<td>"+numbers[index]+"</td>")
							createTable(remainPercents[index]);
						}
					}
				})
				$(".applyBtn").click(function(){
					...
					$.ajax({
						...
						success:function(data){
							...
							var remainPercents = data.result.remainPercents;
							var numbers = data.result.numbers;
							var duration = buildTableFront(new Date(begin),new Date(end));
							for(var index=0;index<remainPercents.length;index++){
								$("#datas").append("<tr></tr>");
								$("#datas").children(":last").append("<td>"+duration[index]+"</td>")
								$("#datas").children(":last").append("<td>"+numbers[index]+"</td>")
			...
			function createTable(datas,begin,end,level){
				for(var index=0;index<9;index++){
					if(datas.remainPercent.length<=index){
						$("#datas").children(":last").append("<td class='colorgrade'></td>");
					}else{
						var percent = datas.remainPercent[index].dailypercent;
						var range = parseInt(percent.replace("%","")/10);
						$("#datas").children(":last").append("<td class='colorgrade"+range+"'>"+percent+"</td>");					
					}
				}
			}
	    </script>
