	...
	<head>
		...
		<script type="text/javascript">
			$(function(){
				$.ajax({
					type:"POST",
					url:"<%=basePath%>admin/checkAllAdmin?pageindex=1",
					dataType:"json",     
					contentType:"application/json",
					success:function(data){
						$("#datas").children().remove();
						var list= data.result.admins;
						if(list.length!=0){
							for(var index=0;index<list.length;index++){
								var admin = list[index];
								createTable(admin.admin_id,admin.id_card,admin.name,admin.email,admin.city,admin.phone,admin.power);
							}
			...
			function search(pageindex){
				var name = $("#search_attribute").val()	;	
				var json = {name:name};	
				$.ajax({
					type:"POST",
					url:"<%=basePath%>admin/checkAdmin?pageindex="+parseInt(pageindex),
					dataType:"json",     
					contentType:"application/json",     
					...
					success:function(data){
						...
						if(list.length!=0){
							...
							$("#account").html(data.result.account)
							ACCOUNT = data.result.account;
							PAGES = parseInt((ACCOUNT+PAGESIZE-1)/PAGESIZE);
							if(PAGES<PAGESIZE)
								createPage(1,PAGES);
							else createPage(1,PAGESIZE);
						}else{
			...
			function createTable(admin_id,id_card,name,email,city,phone,power){
				$("#datas").append("<tr class='"+admin_id+"'></tr>");
				$("#datas").children(":last").append("<td>"+id_card+"</td>");
				$("#datas").children(":last").append("<td>"+name+"</td>");
				$("#datas").children(":last").append("<td>"+email+"</td>");
				$("#datas").children(":last").append("<td>"+city+"</td>");
				$("#datas").children(":last").append("<td>"+phone+"</td>");
				$("#datas").children(":last").append("<td>"+power+"</td>");
				if(power!=1){
					$("#datas").children(":last").append("<td><button type='button' class='btn btn-error btn-sm' onclick=deleteAdmin('"+admin_id+"')>删除管理员</button></td>");
				}else{
					$("#datas").children(":last").append("<td></td>");
				}
			}
			function deleteAdmin(admin_id){
				var json = {admin_id:admin_id}
				alert(admin_id)
				$.ajax({
					type:"POST",
					...
					data:JSON.stringify(json),
					success:function(data){
						$("#datas").children("."+admin_id).remove();
					}
				});
			}
		</script>
			...
			<div class="main-container-inner " >
				...
				<div class="main-content">
					...
					<div class="page-content">
						...
						<div class="row">
							<div class="col-xs-12">
								<h3 class="header smaller lighter blue"><b>全部管理员信息概况</b></h3>
                                <div class="table-header">
                                	<span class="input-icon">
                                        <div class="visible-md visible-lg hidden-sm hidden-xs action-buttons">
                                            <input id="search_attribute" type="text" name="name" placeholder="管理员姓名" class="nav-search-input" id="nav-search-input" autocomplete="off" />
                                            <a class="orange" href="javascript:search(1)">
                            ...
                            <div class="table-foot row">
                                ...
                                <span class="col-md-4 col-sm-offset-5">
